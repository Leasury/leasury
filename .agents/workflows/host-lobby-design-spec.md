...
```
┌──────────────────────────────────────────────────────────────────┐
│  Background: bg-background (dark)  full-screen, centered         │
│                                                                  │
...
- Outer wrapper: `min-h-screen bg-background flex flex-col items-center justify-center p-6 gap-4`
...
### 1. QR Code
- Label above: `text-muted-foreground text-center text-sm mb-4` → `"Scan to join"`
- `<canvas>` element, **250×250 px**, `rounded-md`, centered
- Generated via `qrcode` library: `generateRoomUrl('<game>', room.roomCode, window.location.origin)`
- QR colours: `{ dark: 'var(--foreground)', light: 'var(--card)' }`, margin: 2
...
- Code: `text-4xl font-bold tracking-widest text-foreground tabular-nums font-mono`
...
- **Each player row:** `bg-background px-3 py-2 rounded-md text-sm font-bold text-foreground flex items-center gap-2 border border-border`
  ...
  - Name: `flex-1`
  - _(Optional)_ Kick button: `✕` — `text-muted-foreground hover:text-destructive transition-colors text-lg px-1`
...
### 1. Subtitle
- `text-muted-foreground text-center mb-8` → `"Set up game"`
...
- Label: `"Start Game"` or `"Start Game (N players)"`
...
| `--card` | `oklch(0.970 0.001 106)` ≈ slightly off-white | `oklch(0.235 0.002 107)` ≈ dark card | Panel bg |
| `--foreground` | `oklch(0.191 0.002 107)` ≈ near-black | `oklch(0.982 0.005 95)` ≈ near-white | All text |
| `--muted-foreground` | `oklch(0.321 0 0)` ≈ mid-grey | `oklch(0.895 0.015 102)` ≈ warm light | Labels, hints |
| `--border` | `oklch(0.924 0.014 97)` ≈ warm grey | `oklch(0.284 0.004 107)` ≈ dark border | Dividers |
| `--accent` | `oklch(0.612 0.071 127)` ≈ sage green | `oklch(0.672 0.131 39)` ≈ **terracotta** | Active selection, Start button |
...
- The host lobby always runs in **dark mode** (the TV screen is dark). Use dark token values when reasoning about the actual rendered appearance.
- In dark mode `--accent` = terracotta `oklch(0.672 0.131 39)` — this is the same brand colour (`#D97757`) already used across the platform.
- QR code canvas colours should use CSS variable references or their computed equivalents so they adapt automatically.
