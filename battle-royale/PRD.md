# Battle Royale — Product Requirements Document

## 1. Overview

**Battle Royale** is a turn-based tactical board game for 2–4 players, played on a shared TV/host screen with individual phone controllers. Players move and shoot on a grid (configurable size: 8×8, 10×10, or 12×12) while a shrinking danger zone forces them closer together. The last player standing wins.
...
1. Host creates a room via `/host?game=battle-royale` (or directly at `/games/battle-royale/host`).
...
## 7. Landing Page (`/games/battle-royale`)
...
- **Hero section (left):**
  ...
  - **Buttons** (order depends on device detection — desktop prioritizes Host, mobile prioritizes Join):
    - "Host Battle Royale" → links to `/host?game=battle-royale`.
...
The server broadcasts the full `BattleRoyaleGameState` after every state change, following the existing `{ type: 'sync', room, game }` pattern.
...
```
packages/game-logic/src/games/battle-royale/
  types.ts          # BattleRoyaleGameState, messages, etc.
  logic.ts          # createInitialBattleRoyaleState(), applyBattleRoyaleMessage()
  zone.ts           # Zone shrink algorithm
  resolution.ts     # Turn resolution logic (move, shoot, damage)
  index.ts          # Re-exports

apps/web/app/games/battle-royale/
  page.tsx           # Landing/info page
  host/page.tsx      # Host route
  player/page.tsx    # Player route

apps/web/components/games/battle-royale/
  BattleRoyaleHost.tsx    # Main host component
  BattleRoyalePlayer.tsx  # Main player component
...
apps/server/src/games/
  battle-royale.ts  # Server-side message handler
```

### 13.2 Integration Points

- **Server:** Add `battle-royale` to the `gameType` union. Add `handleBattleRoyaleMessage` handler in `server.ts`. Server manages the planning timer and resolution phase progression.
- **Game logic:** Pure functions in `@lesury/game-logic`. Server calls `applyBattleRoyaleMessage()` for player actions and `advanceBattleRoyalePhase()` for phase transitions.
- **Client:** Uses `usePartyRoom<BattleRoyaleGameState>` hook. Host renders grid + animations. Player renders the Command Queue (D-pad + slots) during planning, event log during resolution.
- **Lobby:** Uses the standard `/host?game=battle-royale` → `RoomHost` → redirect flow.
