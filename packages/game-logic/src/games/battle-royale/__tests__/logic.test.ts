...
function makePlayer(
    ...
    row: number,
    col: number,
    overrides: Partial<BattleRoyalePlayer> = {}
): BattleRoyalePlayer {
    return {
        id,
        name: id,
...
function makeState(overrides: Partial<BattleRoyaleGameState> = {}): BattleRoyaleGameState {
...
describe('createInitialBattleRoyaleState', () => {
    it('creates correct shape', () => {
        const state = createInitialBattleRoyaleState(PLAYER_IDS);
    ...
    it('respects arena size setting', () => {
        const small = createInitialBattleRoyaleState(PLAYER_IDS, 'small');
        const large = createInitialBattleRoyaleState(PLAYER_IDS, 'large');
...
describe('applyBattleRoyaleMessage: start_game', () => {
    it('transitions from lobby to planning', () => {
        const state = makeState({ phase: 'lobby', round: 0 });
        const next = applyBattleRoyaleMessage(state, {
            type: 'start_game',
            arenaSize: 'medium',
            planningDuration: 20,
        });
...
describe('applyBattleRoyaleMessage: submit_actions', () => {
    ...
    it('starts resolution when all alive players submit', () => {
        let state = makeState();
        state = applyBattleRoyaleMessage(
            state,
            { type: 'submit_actions', actions: ACTIONS },
            'p1'
        );
...
describe('applyBattleRoyaleMessage: unlock_actions', () => {
    it('unlocks a locked-in player', () => {
        ...
        const unlocked = applyBattleRoyaleMessage(
            state,
            { type: 'unlock_actions' },
            'p1'
        );
...
describe('applyBattleRoyaleMessage: end_planning', () => {
    it('defaults missing actions and starts resolution', () => {
        const state = makeState();
        const next = applyBattleRoyaleMessage(state, { type: 'end_planning' });
...
describe('applyBattleRoyaleMessage: advance_phase', () => {
    it('advances through resolution steps', () => {
        let state = makeState();
        state = applyBattleRoyaleMessage(state, { type: 'end_planning' });
        const firstPhase = state.phase;
        state = applyBattleRoyaleMessage(state, { type: 'advance_phase' });
    ...
    it('is a no-op in planning phase', () => {
        const state = makeState({ phase: 'planning' });
        const next = applyBattleRoyaleMessage(state, { type: 'advance_phase' });
...
describe('applyBattleRoyaleMessage: play_again', () => {
    it('resets to lobby with same players', () => {
        const state = makeState({ phase: 'game-over', winner: 'p1' });
        const next = applyBattleRoyaleMessage(state, { type: 'play_again' });
